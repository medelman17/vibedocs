...
7. **adminTriggerAnalysis** - Re-trigger analysis on existing document.
   ...
   - Import `inngest` from `@/inngest` -- this is safe per CLAUDE.md: "Existing barrels allowed for lightweight exports: @/inngest (client, utils, types) - safe". The `inngest` client is a lightweight export, not a heavy dependency like functions.
   - `revalidatePath("/admin")`
...
5. Test `adminDeleteDocument` cascade logic -- specifically verify that comparisons where the document is `documentAId` OR `documentBId` are cleaned up before the document is hard-deleted. Create two documents, create a comparison between them, delete one document, verify the comparison row is removed and the other document still exists.
...
Tests pass: `cd /Users/medelman/GitHub/medelman17/vibedocs && pnpm test -- app/\(main\)/\(admin\)/admin/actions.test.ts --run 2>&1 | tail -30`
  ...
  <done>
Admin layout enforces requireRole server-side. Action tests verify pagination, search, filter, hard-delete cascade (including comparison cleanup for both documentAId and documentBId), bulk delete, and analysis deletion without last-analysis guard.
  </done>
</task>
...
- Tests pass for core action logic including comparison cleanup
