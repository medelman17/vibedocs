...
<task type="auto">
  <name>Task 1: Document detail panel and delete dialogs</name>
  <files>
    app/(main)/(admin)/admin/document-detail.tsx
    app/(main)/(admin)/admin/delete-dialog.tsx
  </files>
...
Update `app/(main)/(admin)/admin/page.tsx` to:
- Add client-side state for selected documentId (for detail panel)
- Pass `onRowClick` to DocumentsTable that sets the selected documentId
- Render `<DocumentDetail>` with the selected documentId
- Render `<BulkDeleteDialog>` triggered by toolbar's bulk delete button

Update `app/(main)/(admin)/admin/documents-table.tsx` to:
- Expose selected row IDs to parent (via callback or by lifting row selection state up)
- Wire bulk delete button in toolbar to open BulkDeleteDialog
- After any delete operation, use `router.refresh()` to re-fetch data

This may require making page.tsx a client component wrapper, or creating a thin client wrapper component that manages the detail panel and delete dialog state while keeping the page.tsx as RSC. Recommended approach: Create an `admin-page-client.tsx` "use client" wrapper that receives the server-fetched data and manages all client state (selected row, detail panel open, delete dialogs).
  </action>
  <verify>
TypeScript compiles: `cd /Users/medelman/GitHub/medelman17/vibedocs && npx tsc --noEmit --pretty 2>&1 | head -50`
  </verify>
  <done>
Document detail panel opens on row click, shows document info/actions/analyses. Delete confirmation dialogs exist for both single and bulk delete with proper warning text. All admin actions (edit title, download, re-trigger analysis, delete document, delete analysis, bulk delete) are wired and functional.
  </done>
</task>
