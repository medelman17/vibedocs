...
must_haves:
  truths:
    - "Document appears immediately after upload with progressive reveal as pipeline runs"
    - "URL contains analysis and clause IDs for shareable links"
    - "Analysis route is accessible from proxy/auth middleware"
    - "Full end-to-end flow works: upload -> navigate -> render -> highlight -> navigate"
  artifacts:
...
<task type="auto">
  ...
  <done>Progressive reveal shows document text immediately, adds highlights as analysis completes. URL contains clause ID for shareable links. Auth middleware allows /analysis/ routes. Responsive layout stacks on mobile.</done>
</task>
