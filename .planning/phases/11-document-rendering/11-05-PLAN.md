...
<task type="auto">
  ...
  <action>
    ...
    **page.tsx:**
    ...
    NOTE on AnalysisView API after Plan 11-04 refactor: AnalysisView retains the same public interface (`analysisId: string, className?: string`). Plan 11-04 extracts internal components into tabs but does NOT change the AnalysisView props contract. The refactored AnalysisView internally renders `<AnalysisTabs>` but is consumed here with the same `<AnalysisView analysisId={analysisId} />` call as before the refactor. Read the 11-04-SUMMARY.md to confirm this before implementing.
...
<task type="auto">
  ...
  <action>
    ...
    **chat page navigation update (`app/(main)/chat/page.tsx`):**
    The `handleFileUpload` function is defined at ~line 186 in `app/(main)/chat/page.tsx`. It handles the file upload + triggerAnalysis flow.
