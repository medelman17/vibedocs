...
<task type="auto">
  ...
  <action>
    First install new dependencies and verify installation:
    ```bash
    pnpm add react-markdown @tanstack/react-virtual
    ```
    After install, verify both packages appear in `package.json` dependencies and `node_modules` exists:
    ```bash
    grep -E "react-markdown|@tanstack/react-virtual" package.json
    ```
    If either is missing, the install failed -- retry before proceeding.
  ...
  <action>
    ...
    **document-renderer.tsx:**
    ...
    5. **Paper styling:** Wrap the virtualizer in a container with:
       - `bg-card rounded-lg border shadow-sm` -- uses semantic token so it adapts to dark mode automatically (Claude's Discretion decision: respect app theme rather than always-light, per 11-CONTEXT.md)
