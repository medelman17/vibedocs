...
```
  ...
  <done>
    ...
    - Server action triggers re-score via Inngest event
...
<task type="auto">
  <name>Task 2: Add risk scoring UI components to analysis view</name>
  <files>components/artifact/analysis-view.tsx</files>
  <action>
Enhance `components/artifact/analysis-view.tsx` with risk scoring UI. Add to the existing component structure (do NOT rewrite the entire file -- add new sections):
...
3. **Risk Assessment List** (enhance the existing clause list OR add a "Risk" tab/view):
...
4. **Evidence Expandable** (for each risk assessment):
...
5. **Imports to add**:
```typescript
import { triggerRescore, getRiskAssessments } from "@/app/(main)/(dashboard)/analyses/actions"
```
...
- Poll for completion: after triggering rescore, poll `getAnalysis` every 3 seconds until progressStage returns to 'complete'
  ...
  <verify>
    - `pnpm build` succeeds
    - `pnpm lint` passes
    - Analysis view shows perspective toggle, executive summary, risk distribution
    - Evidence expandable renders citations and references
    - Risk level badges use correct colors
  </verify>
  <done>
    ...
    - Each clause shows risk level, explanation, and optional negotiation suggestion
