...
must_haves:
  truths:
    - "Per-clause risk assessments are persisted to clauseExtractions table"
    - "Document-level risk score uses category importance weighting from cuadCategories.riskWeight"
    - "Executive summary stored in analyses.summary column"
    - "Perspective stored in analyses.metadata"
    - "Risk distribution (count per level) stored in analyses metadata"
  artifacts:
