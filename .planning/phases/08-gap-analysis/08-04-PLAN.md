...
must_haves:
  ...
  key_links:
    - from: "components/artifact/analysis-view.tsx"
      ...
      via: "fetchGapAnalysis import and call, returns ApiResponse<EnhancedGapResult>"
    ...
    - from: "app/(main)/(dashboard)/analyses/actions.ts"
      ...
      via: "getGapAnalysis returns EnhancedGapResult from JSONB column"
...
**5. Add `GapsView` component** (follows `ClassificationView` pattern — useEffect fetch on mount):
...
- **End-to-end data flow**: `fetchGapAnalysis(analysisId)` returns `ApiResponse<EnhancedGapResult>` with typed `gaps`, `coverageSummary`, `presentCategories`, `weakClauses`, `hypothesisCoverage`, and `gapScore` fields. Verify with: `grep -n "EnhancedGapResult" components/artifact/analysis-view.tsx app/(main)/(dashboard)/analyses/actions.ts db/queries/gap-analysis.ts agents/types.ts` -- all 4 files should reference the type.
</verification>
