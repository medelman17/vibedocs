...
Add JSDoc explaining that voyage-law-2 uses Llama 2 tokenizer per Voyage AI docs, and that gpt-tokenizer undercounts by 10-20%.
  </action>
  <verify>
...
4. `analysisId: uuid("analysis_id").references(() => analyses.id, { onDelete: "cascade" })` - link to specific analysis run
5. `overlapTokens: integer("overlap_tokens").default(0)` - how many tokens are overlap from previous chunk

IMPORTANT: You will need to import `analyses` from `./analyses` in `db/schema/documents.ts`. Watch for circular imports -- the analyses schema already imports `documents` and `documentChunks` from this file. To avoid circular dependency, use a string reference or restructure. The safest approach: do NOT add a foreign key reference to analyses. Instead, store analysisId as a plain `uuid("analysis_id")` column without `.references()`. The application layer will enforce this relationship. This avoids the circular import.
...
Run `npx tsc --noEmit` to verify no circular import issues or type errors. Run `pnpm db:generate` and check that a migration file is created in `./drizzle/` with the expected ALTER TABLE statements.
  ...
  <done>
- documentChunks has startPosition, endPosition, chunkType, analysisId, overlapTokens columns
...
- No circular dependency between documents.ts and analyses.ts
</success_criteria>
