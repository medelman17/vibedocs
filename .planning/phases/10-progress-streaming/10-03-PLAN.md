  ...
  <action>
    ...
    5. Rewrite the hook body:
       ...
       b. **Inngest Realtime subscription** (primary transport):
          - Use `useInngestSubscription` with `refreshToken` callback:
            ```typescript
            const subscription = useInngestSubscription({
              refreshToken: analysisId ? () => fetchRealtimeToken(analysisId) : undefined,
              enabled: !!analysisId && !isTerminal,
            })
            ```
            Where `isTerminal` is derived from current state (completed/failed/cancelled).
    ...
    - The `enabled` option may not exist on `useInngestSubscription` — if not, conditionally pass `refreshToken` as `undefined` when disabled (the hook should no-op without a token fetcher)
