...
<task type="auto">
   ...
   *
   * NOTE ON DUPLICATE STATUS UPDATES: The cancelAnalysis server action
   * optimistically sets status='cancelled' in the DB for immediate UI
   * feedback. This cleanup handler ALSO sets status='cancelled'. Both
   * updates are intentional and idempotent:
   * - Optimistic update: Gives the user instant UI feedback
   * - Cleanup handler: Acts as a safety net in case the server action
   *   errored after sending the cancellation event (e.g., DB write
   *   failed but Inngest event was already dispatched). Both writing
   *   the same value ('cancelled') is a no-op, not a conflict.
   *
...
<task type="auto">
  ...
  **Update cancelAnalysis in actions.ts:**
  ...
  export async function cancelAnalysis(
    ...
    // The cleanup handler (cleanup-cancelled.ts) will also set
    // status='cancelled' as a safety net — both updates are
    // idempotent (same value). See cleanup handler docs for rationale.
    await db
      .update(analyses)
      .set({
        status: "cancelled",
  ...
  **Add resumeAnalysis action:**
   ...
   *
   * IMPLEMENTATION NOTE: "Resume" is implemented by sending a new
   * `nda/analysis.requested` event, which creates a fresh Inngest run.
   * This is the correct Inngest pattern — there is no "resume from step X"
   * API. Inngest step memoization means that completed step.run() calls
   * return their cached results instantly without re-executing, so the
   * pipeline effectively skips completed steps and picks up at the first
   * uncompleted step. The result is functionally identical to "resume from
   * the failed step" even though the mechanism is replay-based.
   *
