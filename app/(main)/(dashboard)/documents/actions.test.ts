...
describe("documents/actions", () => {
  ...
  describe("getDocuments", () => {
    ...
    it("filters by status when provided", async () => {
      ...
      await createTestDocument(org.id, { title: "Complete Doc", status: "complete" })
      await createTestDocument(org.id, { title: "Pending Doc", status: "pending" })
    ...
    it("excludes soft-deleted documents", async () => {
      ...
      await createTestDocument(org.id, { title: "Deleted Doc", deletedAt: new Date() })
  ...
  describe("getDocument", () => {
    ...
    it("returns NOT_FOUND for non-existent document", async () => {
      ...
      const { getDocument } = await import("./actions")
      const result = await getDocument({ documentId: "00000000-0000-0000-0000-000000000000" })
  ...
  describe("getDashboardStats", () => {
    ...
    it("excludes soft-deleted documents from counts", async () => {
      ...
      await createTestDocument(org.id, { status: "complete" })
      await createTestDocument(org.id, { status: "complete", deletedAt: new Date() })
