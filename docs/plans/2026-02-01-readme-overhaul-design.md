...
- **Primary audience:** Recruiters and technical evaluators (portfolio showcase)
- **Lead with:** Architecture-forward (system diagrams, tech stack decisions, patterns)
- **Key differentiators:** Two-tier data model, agent pipeline design, RAG implementation (all equally)
- **Visual style:** Badge-focused (professional open-source project look)
- **Structure:** Layered disclosure with collapsible `<details>` sections
...
[![Build Status](https://img.shields.io/github/actions/workflow/status/medelman17/nda-analyst/ci.yml?branch=main)](https://github.com/medelman17/nda-analyst/actions)
...
```bash
git clone https://github.com/medelman17/nda-analyst.git
cd nda-analyst
pnpm install
...
## Section 7: Project Structure & Closing
...
| [schema.md](docs/schema.md) | Database schema with RLS policies |
| [agents.md](docs/agents.md) | Agent pipeline specifications |
...
1. Update badge URLs to match actual repo path
2. Verify all doc links exist before merging
3. Consider adding a hero screenshot once UI is built
4. Build status badge requires CI workflow to be configured
